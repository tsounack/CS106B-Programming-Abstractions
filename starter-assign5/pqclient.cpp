...
Vector<DataPoint> topK(istream& stream, int k) {
    ...
    // Creating the priority queue with the right
    // number of elements
    DataPoint cur;
    while (stream >> cur) {
        pq.enqueue(cur);
        // we only need to dequeue if we are above k
        if (pq.size() > k) {
            pq.dequeue();
        }
    }

    int lenResults = pq.size();

    // This way of adding results to the vector ensures
    // it is done in a O(k) complexity.
    Vector<DataPoint> results(lenResults);
    for (int i = lenResults - 1; i >= 0; i--) {
...
STUDENT_TEST("topK: time trial n, keeping k constant") {
...
STUDENT_TEST("topK: time trial k, keeping n constant") {
    int n = 200000;
    int startK = 1000;
